...
let structureViewer;
let structureViewerLoading = false;
...
function commitAppView(view) {
  if (view !== 'design') structureViewer?.plugin?.animationLoop?.stop({ noDraw: true });
  ...
  location.hash = view;
  document.querySelectorAll('[data-project-topic]').forEach((button) => button.classList.toggle('is-active', button.dataset.projectTopic === view));
...
function afterAppViewChange(view) {
  ...
  if (view === 'design') requestAnimationFrame(() => {
    if (!structureViewer) initialiseStructureViewer();
    else syncStructureActivity();
  });
...
function openProjectTopic(topicId) {
  ...
  if (destination.section) requestAnimationFrame(() => document.getElementById(destination.section)?.scrollIntoView({ behavior: 'smooth', block: 'start' }));
}
...
function bindViewerTabs() {
  document.querySelectorAll('[data-viewer-tab]').forEach((button) => button.addEventListener('click', () => {
    ...
    if (tab !== 'structure') structureViewer?.plugin?.animationLoop?.stop({ noDraw: true });
    ...
    if (tab === 'structure') requestAnimationFrame(syncStructureActivity);
    if (tab === 'pae') loadPae();
  }));
}
...
function syncStructureActivity() {
  const host = document.getElementById('molstar-app');
  const visible = host && host.getBoundingClientRect().width > 0 && host.getBoundingClientRect().height > 0;
  if (!visible) structureViewer?.plugin?.animationLoop?.stop({ noDraw: true });
  else {
    structureViewer?.plugin?.canvas3d?.requestResize?.();
    structureViewer?.plugin?.animationLoop?.start();
  }
}
